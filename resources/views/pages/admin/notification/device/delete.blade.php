...
<div class="max-w-md mx-auto text-center bg-white px-4 sm:px-8 py-10 rounded-xl shadow mt-40">
    <header class="mb-8">
    ...
    <form method="post" id="otp-form" action="{{route('super-admin.wa-gateway.device.destroy', ['token' => $token])}}">
        @csrf
        @method('DELETE')
    ...
    <form action="{{route('super-admin.wa-gateway.device.delete-request', ['token' => $token])}}">
        <div class="text-sm text-slate-500 mt-4">Didn't receive code?
            <button type="submit"
                    class="font-medium text-indigo-500 hover:text-indigo-600">Resend
            </button>
        </div>
    </form>
</div>
