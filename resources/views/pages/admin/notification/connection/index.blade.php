...
@section('title', 'WA Gateway Connection')

@section('content')
    <form class="max-w-sm mx-auto mt-20" method="post"
          action="{{route('super-admin.wa-gateway.connection.connection-post')}}">
        @csrf
        @if (session('success'))
        ...
        <div class="max-w-sm mb-5">
            <label class="block text-sm mb-2 dark:text-white">Fonnte Account Token</label>
            <div class="relative">
                <input id="hs-toggle-password" type="password" name="token"
                       ...
                       placeholder="Your Fonnte Account Token" value="{{env('FONNTE_ACCOUNT_TOKEN')}}">
